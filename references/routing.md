...
主路线和范围确认后，若资料包含可观测指标且问题要求诊断、解释、预测、干预效果或行动选择，读取 [deep-data-analysis-kernel.md](deep-data-analysis-kernel.md) 并运行 `compile-question`。它不是新的主路线，而是在方法选择前区分测量、描述、时间、异质性、机制、因果、预测和决策层，防止用同一套统计回答不同性质的问题。纯质性资料和简单查数不调用。
